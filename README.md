...
成果・集中・品質の実績入力が揃う前は、現在の睡眠・食事・デジタル入力から推定パフォーマンスを表示します。推定値は画面上で「推定」と入力カバー率を明示し、実績スコアや実績平均とは混ぜません。
...
列定義は[`docs/技術要件書.md`](./docs/技術要件書.md)の「データモデルとGoogle Spreadsheet」を参照してください。`daily_logs`には画面の詳細状態を復元するための`payload_json`列に加え、推定を実績と分離して追跡する3列も使用します。
