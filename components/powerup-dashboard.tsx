...
function blankSnack(date: string): SnackInput {
  return { id: `${date}-snack-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, eatenAt: null, occurred: true, category: null, amountLevel: null, planned: null, beforeBed: false, note: "" };
}
  ...
  return (
    <main className="page-content">
      ...
      <section className="card score-hero" aria-label="今日のパフォーマンススコア">
        <div className="score-hero-main"><div className="score-hero-title"><span className="pulse-dot" /> 今日のパフォーマンススコア</div><div className="score-figure"><strong>{todayScore ?? "—"}</strong><span>/ 100</span></div><p className="score-label">{todayScore === null ? "成果・集中・品質を記録しましょう" : todayScore >= 80 ? "良い流れです。" : "振り返る材料が見えてきました。"}</p><p className="score-subline">{scores.provisional ? `成果入力カバー率 ${scores.recordingRate}% · 未入力は推定しません` : "成果・集中・品質評価から算出"}</p></div>
      ...
      <div className="metric-grid">
        ...
        <MetricCard icon="↗" tone="result" title="実際のパフォーマンス" value={displayScore(scores.result, SCORE_MAX.result)} meta={input.result.focusMinutes === null ? "成果・集中・品質を入力" : `集中 ${input.result.focusMinutes}分`} score={scores.result} max={SCORE_MAX.result} />
      </div>
...
function Editor({ input, scores, aiInsight, aiLoading, saving, message, error, updateInput, updateMeal, updateSnack, askGemini, adoptAi, save }: EditorProps) {
  return <section className="card editor-card" id="settings"><div className="editor-heading"><div><h2>記録を編集</h2><p>選択式中心。Geminiが配点案をつくり、最後はあなたが確定します。</p></div><span className="ai-badge">Gemini API preview</span></div>
    ...
    <div className="editor-actions">{message ? <span className="save-message" role="status">{message}</span> : error ? <span className="error-message" role="alert">{error}</span> : <span className="save-message">パフォーマンススコア：{scores.total ?? "未確定"} / 100</span>}<button className="ghost-button" type="button" onClick={() => window.location.reload()}>入力を戻す</button><button className="primary-button" type="button" onClick={save} disabled={saving}>{saving ? "保存しています…" : "今日のログを保存"}</button></div>
  </section>;
}
