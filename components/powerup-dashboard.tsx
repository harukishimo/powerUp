...
import { calculateMealPoints, calculateScores } from "@/lib/scoring";
...
function blankSnack(date: string): SnackInput {
  return { id: `${date}-snack-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, eatenAt: null, occurred: true, category: "healthy-small", amountLevel: "small", planned: true, beforeBed: false, note: "" };
}
...
export function PowerUpDashboard({ initialLog, initialSummaries, configurationWarning = false }: { initialLog: DailyLog; initialSummaries: DailyLogSummary[]; configurationWarning?: boolean }) {
  ...
  const todayScore = scores.total;
  const trend = buildWeeklyTrend(summaries, input.date);
  const average = summaries.length ? Math.round(summaries.reduce((total, item) => total + (item.totalScore ?? 0), 0) / summaries.length) : 0;

  return (
    <main className="page-content">
      <div className="page-heading">
        <div><p className="eyebrow">{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" }).format(new Date(`${input.date}T12:00:00+09:00`)).toUpperCase()}</p><h1>今日のコンディション</h1><p>小さな選択の積み重ねが、集中の土台になります。</p></div>
      ...
      <section className="card score-hero" aria-label="今日の総合スコア">
        <div className="score-hero-main"><div className="score-hero-title"><span className="pulse-dot" /> 今日の総合スコア</div><div className="score-figure"><strong>{todayScore ?? "—"}</strong><span>/ 100</span></div><p className="score-label">{todayScore === null ? "まずは1項目を記録しましょう" : todayScore >= 80 ? "良い流れです。" : "整えるポイントが見えてきました。"}</p><p className="score-subline">{scores.provisional ? `記録率 ${scores.recordingRate}% · 入力済み項目から算出` : "4カテゴリの記録から算出"}</p></div>
        <div className="score-hero-chart"><div className="chart-header"><strong>直近7日間の流れ</strong><span>平均 <b>{average || "—"}</b></span></div><div className="trend-bars">{trend.map((bar) => <div className={`trend-column ${bar.today ? "today" : ""}`} key={bar.label}><div className="trend-bar" style={{ height: `${Math.max(7, (bar.value / 100) * 80)}px` }} /><span>{bar.label}</span></div>)}</div><div className="trend-average"><span>今日の記録を続けるほど、傾向が見えます。</span></div></div>
      </section>
      <div className="metric-grid">
        <MetricCard icon="◒" tone="sleep" title="睡眠" value={displayScore(scores.sleep, 50)} meta={input.sleep.pixelWatchScore === null ? "Pixel Watchスコアを入力" : `デバイススコア ${input.sleep.pixelWatchScore}`} score={scores.sleep} max={50} />
        <MetricCard icon="◌" tone="food" title="食事" value={displayScore(scores.food, 30)} meta={`${input.meals.filter((meal) => meal.eatenAt).length}食 · 間食 ${input.snackRecorded ? "記録済み" : "未記録"}`} score={scores.food} max={30} />
        <MetricCard icon="▣" tone="phone" title="スマートフォン" value={displayScore(scores.phone, 10)} meta={input.phone.entertainmentMinutes === null ? "利用時間を入力" : `娯楽 ${Math.floor(input.phone.entertainmentMinutes / 60)}h ${input.phone.entertainmentMinutes % 60}m`} score={scores.phone} max={10} />
        <MetricCard icon="↗" tone="result" title="実際の成果" value={displayScore(scores.result, 10)} meta={input.result.focusMinutes === null ? "今日の成果を入力" : `集中 ${input.result.focusMinutes}分`} score={scores.result} max={10} />
      </div>
...
function Editor({ input, scores, aiInsight, aiLoading, saving, message, error, updateInput, updateMeal, updateSnack, askGemini, adoptAi, save }: EditorProps) {
  return <section className="card editor-card" id="settings"><div className="editor-heading"><div><h2>記録を編集</h2><p>選択式中心。Geminiが配点案をつくり、最後はあなたが確定します。</p></div><span className="ai-badge">Gemini API preview</span></div>
    <div className="editor-section"><div className="editor-section-title"><span className="section-number">01</span>睡眠</div><div className="input-grid two"><Field label="Pixel Watch 睡眠スコア"><input type="number" min="0" max="100" value={input.sleep.pixelWatchScore ?? ""} placeholder="0〜100" onChange={(event) => updateInput({ sleep: { ...input.sleep, pixelWatchScore: event.target.value === "" ? null : Number(event.target.value) } })} /></Field><Field label="朝の回復感"><ChoiceGroup name="sleep-recovery" value={input.sleep.recoveryFeeling?.toString() ?? ""} options={["1", "2", "3", "4", "5"]} labels={["1", "2", "3", "4", "5"]} onChange={(value) => updateInput({ sleep: { ...input.sleep, recoveryFeeling: value ? Number(value) : null } })} /></Field></div><p className="help-text">睡眠ポイント：{scores.sleep === null ? "未記録" : `${scores.sleep} / 50`} · デバイススコアはユーザーが毎回入力します。</p></div>
    <div className="editor-section"><div className="editor-section-title"><span className="section-number">02</span>食事と間食</div>{(["breakfast", "lunch", "dinner"] as MealType[]).map((type) => <MealEditor key={type} meal={input.meals.find((item) => item.type === type) ?? blankMeal(type)} update={(patch) => updateMeal(type, patch)} />)}<div className="meal-panel"><div className="meal-panel-head"><strong>間食</strong><span className="meal-score">{input.snackRecorded ? `${scores.food === null ? "—" : "記録済み"}` : "未記録"}</span></div><div className="choice-group"><Choice name="snack-status" label="間食なし" checked={input.snackRecorded && input.snacks.length === 0} onChange={() => updateInput({ snackRecorded: true, snacks: [] })} /><Choice name="snack-status" label="間食あり" checked={input.snackRecorded && input.snacks.length > 0} onChange={() => updateInput({ snackRecorded: true, snacks: input.snacks.length ? input.snacks : [blankSnack(input.date)] })} /><Choice name="snack-status" label="未記録" checked={!input.snackRecorded} onChange={() => updateInput({ snackRecorded: false, snacks: [] })} /></div>{input.snackRecorded && input.snacks.length > 0 ? <div className="snack-list" style={{ marginTop: 11 }}>{input.snacks.map((snack, index) => <div className="snack-row" key={snack.id ?? `${input.date}-snack-${index}`}><div className="input-grid four"><Field label="時刻"><input type="time" value={snack.eatenAt ?? ""} onChange={(event) => updateSnack(index, { eatenAt: event.target.value || null })} /></Field><Field label="内容"><select value={snack.category ?? ""} onChange={(event) => updateSnack(index, { category: (event.target.value || null) as SnackInput["category"] })}><option value="">選択</option><option value="healthy-small">ナッツ・ヨーグルト・果物</option><option value="planned-meal">計画した補食</option><option value="sweet-only">菓子・甘い飲料</option><option value="large-or-late">大量・就寝前</option></select></Field><Field label="量"><select value={snack.amountLevel ?? ""} onChange={(event) => updateSnack(index, { amountLevel: (event.target.value || null) as SnackInput["amountLevel"] })}><option value="">選択</option><option value="small">少量</option><option value="medium">普通</option><option value="large">多い</option></select></Field><Field label="メモ"><input value={snack.note} onChange={(event) => updateSnack(index, { note: event.target.value })} placeholder="任意" /></Field></div><div className="snack-actions"><label className="check-label"><input type="checkbox" checked={snack.planned === true} onChange={(event) => updateSnack(index, { planned: event.target.checked })} />計画的</label><button className="text-button" type="button" onClick={() => updateInput({ snacks: input.snacks.filter((_, snackIndex) => snackIndex !== index) })}>削除</button></div></div>) }<button className="ghost-button" type="button" onClick={() => updateInput({ snacks: [...input.snacks, blankSnack(input.date)] })}>＋間食を追加</button></div> : null}</div><div className="meal-panel"><div className="meal-panel-head"><strong>食事の時間帯</strong><span className="meal-score">{scores.food === null ? "—" : `${scores.food} / 30`}</span></div><div className="check-row"><Check label="時刻が大きく乱れなかった" value={input.mealTiming.regular} onChange={(value) => updateInput({ mealTiming: { ...input.mealTiming, regular: value } })} /><Check label="夕食が就寝直前ではない" value={input.mealTiming.dinnerBeforeBed} onChange={(value) => updateInput({ mealTiming: { ...input.mealTiming, dinnerBeforeBed: value } })} /><Check label="長時間の空腹後に食べ過ぎなかった" value={input.mealTiming.noLongGap} onChange={(value) => updateInput({ mealTiming: { ...input.mealTiming, noLongGap: value } })} /></div></div></div>
    <div className="editor-section"><div className="editor-section-title"><span className="section-number">03</span>スマートフォン</div><div className="input-grid"><Field label="娯楽利用時間（分）"><input type="number" min="0" max="1440" value={input.phone.entertainmentMinutes ?? ""} placeholder="例：120" onChange={(event) => updateInput({ phone: { ...input.phone, entertainmentMinutes: event.target.value === "" ? null : Number(event.target.value) } })} /></Field><div className="field"><label>利用の工夫</label><div className="check-row"><Check label="作業中に離した" value={input.phone.separatedDuringWork} onChange={(value) => updateInput({ phone: { ...input.phone, separatedDuringWork: value } })} /><Check label="朝夜を抑えた" value={input.phone.limitedMorningOrNightUse} onChange={(value) => updateInput({ phone: { ...input.phone, limitedMorningOrNightUse: value } })} /></div></div></div><p className="help-text">スマートフォンポイント：{scores.phone === null ? "未記録" : `${scores.phone} / 10`}</p></div>
    <div className="editor-section"><div className="editor-section-title"><span className="section-number">04</span>実際の成果</div><div className="field"><label htmlFor="achievement">今日、実際に進んだこと（1〜2文）</label><textarea id="achievement" value={input.result.achievementText} maxLength={240} onChange={(event) => updateInput({ result: { ...input.result, achievementText: event.target.value } })} placeholder="例：企画書の構成を完成させ、レビュー依頼を送った。" /></div><div className="input-grid" style={{ marginTop: 12 }}><Field label="確定する成果点"><ChoiceGroup name="achievement-score" value={input.result.confirmedAchievementScore?.toString() ?? ""} options={["0", "1", "2", "3", "4", "5"]} labels={["0", "1", "2", "3", "4", "5"]} onChange={(value) => updateInput({ result: { ...input.result, confirmedAchievementScore: value ? Number(value) : null } })} /></Field><Field label="集中作業時間（分）"><input type="number" min="0" max="1440" value={input.result.focusMinutes ?? ""} placeholder="例：75" onChange={(event) => updateInput({ result: { ...input.result, focusMinutes: event.target.value === "" ? null : Number(event.target.value) } })} /></Field><Field label="今日の手応え"><ChoiceGroup name="reflection" value={input.result.reflectionRating?.toString() ?? ""} options={["1", "2", "3", "4", "5"]} labels={["1", "2", "3", "4", "5"]} onChange={(value) => updateInput({ result: { ...input.result, reflectionRating: value ? Number(value) : null } })} /></Field></div><div className="field" style={{ marginTop: 12 }}><label htmlFor="comment">振り返りコメント（任意）</label><input id="comment" value={input.result.comment} maxLength={240} onChange={(event) => updateInput({ result: { ...input.result, comment: event.target.value } })} placeholder="一言、二言で今日の感覚を残す" /></div><p className="help-text">成果ポイント：{scores.result === null ? "未記録" : `${scores.result} / 10`}</p></div>
    <div className="editor-section"><div className="editor-section-title"><span className="section-number">✦</span>Gemini採点案</div>{aiInsight ? <div className="ai-box"><div><strong>成果の提案点：<span className="proposal-score">{aiInsight.achievementScore} / 5</span></strong><p>{aiInsight.reason}<br />{aiInsight.nextExperiment}</p></div><div className="ai-box-actions"><button className="small-button" type="button" onClick={adoptAi}>採用する</button><button className="ghost-button" type="button" onClick={askGemini} disabled={aiLoading}>再作成</button></div></div> : <div className="ai-box"><div><strong>入力内容から採点案を作成できます。</strong><p>Geminiは提案を出しますが、確定するのはあなたです。</p></div><button className="small-button" type="button" onClick={askGemini} disabled={aiLoading}>{aiLoading ? "作成中…" : "Gemini採点案"}</button></div>}</div>
    <div className="editor-actions">{message ? <span className="save-message" role="status">{message}</span> : error ? <span className="error-message" role="alert">{error}</span> : <span className="save-message">総合スコア：{scores.total ?? "未確定"} / 100</span>}<button className="ghost-button" type="button" onClick={() => window.location.reload()}>入力を戻す</button><button className="primary-button" type="button" onClick={save} disabled={saving}>{saving ? "保存しています…" : "今日のログを保存"}</button></div>
  </section>;
}
