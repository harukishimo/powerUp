...
| `/habits` | Server + Client Components | 月間の継続ログ、達成チェック、連続回数 | 必須 |
| `/habits/manage` | Server + Client Components | 継続項目の追加・編集・休止・再開 | 必須 |
| `/focus` | Server + Client Components | PC版YouTubeの14日間集中改善プログラム | 必須 |
...
利用者向けの採点系統は5種類（実際のパフォーマンス、睡眠、食事、デジタル注意環境、推定パフォーマンス）とする。`lib/scoring.ts`には、内訳を含め14個の公開計算関数を置く。

実績は成果55・集中30・品質15で計100点とする。現在推定は睡眠準備度30・起床後時間10・現在の覚醒感15・作業／予定10・直近の食事20・デジタル注意環境15で計100点とする。欠測は中立値50へ縮約し、入力済み条件だけで100点へ再配分しない。睡眠スコア75超・覚醒感3超・起床後1〜6時間の条件が揃う場合に限り、睡眠と覚醒感の既存配点枠内で最大3点の実験的な相乗加点を行う。昼寝の開始・終了・評価時刻が揃う場合は、10〜90分の昼寝に限って睡眠慣性と効果持続時間を`-4〜+1`点で補正する。推定モデル版`current-condition-v4`を保存する。詳細式は[`docs/採点ロジックレビュー.md`](./採点ロジックレビュー.md)を正とする。
...
| `habit_master` | 継続項目、対象曜日、表示色、利用状態 | `habit_id` |
| `habit_logs` | 継続項目の日別達成状態 | `habit_id + log_date` |
| `focus_logs` | 14日間の集中改善プログラム、欲求・目的・PC環境の記録 | `user_key + log_date` |
...
`payload_json`には、その日の行動チェック、PC環境設定、欲求トリガーと復帰結果、YouTubeを開く目的メモ、振り返りを保存する。Day1比較に使う主要指標は個別列にも保持し、一覧・分析時にJSON全体を解析しなくても取得できるようにする。

### 6.7 Sheets APIの保存ルール

- Google Sheets APIはサーバー側からのみ使用する
- `user_key + log_date`を検索して既存日次行を更新し、なければ追加する
- 食事は`meal_type`を含むキーで更新する
- 間食は一意な`snack_id`で追加・更新する
- 集中改善ログは`user_key + log_date`で更新し、同日行を重複させない
- ヘッダー行を毎回上書きしない
- 保存前にZodで入力を検証する
