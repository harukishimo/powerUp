...
総合スコアは常に100点満点の固定配点で計算する。未入力カテゴリはスコアへ加算せず、入力済みカテゴリの獲得点だけを合計する。未入力の範囲は`recording_rate`で別に返し、暫定値であることをUIに表示する。

```text
表示スコア = 入力済みカテゴリの獲得点の合計
記録率 = round(入力済み満点 / 100 × 100)
```

例：睡眠が33点（50点満点）のみ入力されている場合、総合スコアは33/100、記録率は50%となる。「表示スコア」は暫定値であり、未入力を0点として確定したものではないことをUIで示す。
