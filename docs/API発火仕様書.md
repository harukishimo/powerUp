...
- テキスト・数値の入力途中はReactのローカル状態で保持し、APIを呼び出さない
...
| `/api/habits` | GET / POST | 継続項目の一覧取得・追加 | 継続画面の表示、項目追加 |
| `/api/habits/[id]` | PATCH | 継続項目の編集・休止・再開 | マスタ画面の明示操作 |
| `/api/habit-logs` | GET / POST | 期間内の達成記録取得・日別達成更新 | 継続カレンダー |
| `/api/focus` | GET / PUT | 14日間の集中改善ログ取得・日別upsert | 集中リセット画面 |
...
| 継続カレンダーを初めて表示 | サーバー側でSheets取得 | 継続項目と当月までの達成記録が未取得 | 1回 |
| 継続マスタを初めて表示 | サーバー側でSheets取得 | 休止中を含む継続項目が未取得 | 1回 |
| 集中リセットを初めて表示 | サーバー側でSheets取得 | 14日分の記録が未取得 | 1回 |
...
| UIイベント | API | 発火条件 | 発火回数 |
|---|---|---|---:|
| 継続項目を追加 | `POST /api/habits` | 名称と対象曜日の検証に成功 | 1回 |
| 継続項目を編集・休止・再開 | `PATCH /api/habits/[id]` | ユーザーが明示的に操作 | 1回 |
| カレンダーの月を変更 | `GET /api/habit-logs` | 表示対象月が変わった | 1回 |
| 日別の達成状態を切り替え | `POST /api/habit-logs` | 対象日・項目の検証に成功 | 1回 |

休止はマスタ行の`active`だけを更新し、過去の`habit_logs`は削除しない。対象外の曜日は連続回数を途切れさせず、当日の未達成は日付が変わるまでは現在の連続回数を維持する。

### 4.6 集中リセット

| UIイベント | API | 発火条件 | 発火回数 |
|---|---|---|---:|
| テキスト・数値を入力 | なし | 入力途中 | 0回 |
| 「今日の記録を保存」を押す | `PUT /api/focus` | 対象日が当日以前で入力検証に成功 | 1回 |
| 今日の行動・PC環境チェックを切り替える | `PUT /api/focus` | 明示的なチェック操作 | 1回 |
| YouTube目的を記録 | `PUT /api/focus` | 目的が1文字以上 | 1回 |
| 欲求と結果を記録 | `PUT /api/focus` | トリガーと結果を選択 | 1回 |
| 未来日の内容を見る | なし | 読み取り専用 | 0回 |

`focus_logs`は`user_key + log_date`でupsertし、行動チェック、PC環境、日次指標、欲求ログ、目的メモを`payload_json`にまとめて保存する。1回の保存で同じ日付の重複行を作らない。
...
14日間の対象日について、日次ログ全体をupsertする。未来日は保存できない。

```json
{
  "date": "2026-08-12",
  "firstTask": "エラー文をコピーする",
  "completedActionIds": ["install-unhook"],
  "environmentCheckIds": ["homepage-feed", "shorts", "autoplay"],
  "metrics": {
    "aimlessOpenCount": 3,
    "workYoutubeMinutes": 25,
    "shortsMinutes": 0,
    "workStartDelayMinutes": 12,
    "recognizedUrges": 2,
    "returnedToWorkCount": 1
  },
  "urgeEvents": [],
  "purposeEntries": [],
  "reflection": "難しい調査の直前に開きたくなった。"
}
```
...
- 集中リセットは同日保存を繰り返しても`focus_logs`に重複行を作らない
- 集中リセットの未来日は読み取り専用になる
